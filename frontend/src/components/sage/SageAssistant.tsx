...
import sageImage from "@/assets/sage-assistant.png";
import { Button } from "@/components/ui/button";
import { type ChatMessage, type ToolActivity, useSageChat } from "@/components/sage/useSageChat";
...
/** Sage, the floating cooking companion. Reads the cookbook through the
 *  assistant service and proposes drafts; it never writes a recipe itself. */
export function SageAssistant() {
  const pathname = useRouterState({ select: (s) => s.location.pathname });
  const [open, setOpen] = useState(false);
  const [draft, setDraft] = useState("");
  const inputRef = useRef<HTMLTextAreaElement>(null);
  const logRef = useRef<HTMLDivElement>(null);
  const { messages, status, health, checkHealth, send, stop, reset } = useSageChat();
  ...
  return (
    <div className="assistant-wrap">
      {open && (
        <div className="assistant-panel glass-panel" role="dialog" aria-label="Sage assistant">
          <div className="assistant-head">
            <img src={sageImage} alt="" width={816} height={816} />
            <div className="min-w-0 flex-1">
              <h2 className="font-display text-lg font-semibold">Sage</h2>
            ...
            <Button size="icon-sm" variant="ghost" onClick={() => setOpen(false)} aria-label="Close Sage">
              <X />
            </Button>
          </div>
          ...
          {down && (
            <div className="assistant-status is-down flex items-center gap-1.5">
              <AlertCircle className="size-3.5" /> Sage is currently unavailable{health?.detail ? ` — ${health.detail}` : ""}. Everything else still works.
            </div>
          )}
          ...
          <div ref={logRef} className="assistant-log">
            {messages.length === 0 && (
              <div className="assistant-greeting">
                <p className="font-medium">Hi! I'm Sage, your cooking companion.</p>
          ...
          >
            <textarea
              ...
              placeholder={down ? "Sage is offline" : "Ask Sage about dinner…"}
      ...
      <Button className="assistant-toggle" onClick={() => setOpen((v) => !v)} aria-label={open ? "Close Sage assistant" : "Open Sage assistant"}>
        {open ? <X /> : <MessageCircle />}
        <span>{open ? "Close" : "Ask Sage"}</span>
      </Button>
    </div>
  );
}
  ...
  return (
    <div className="msg msg-assistant flex flex-col gap-2">
      ...
      {waiting && message.activity.every((a) => a.status !== "running") && (
        <span className="typing-dots text-muted-foreground" aria-label="Sage is thinking">
          <span />
          <span />
          <span />
        </span>
